...
L putf_: {
    ...
    if(R3 > -1){
        int a = 0;
        while(a < R3){
            printf((char*)p,U(R2+a*4));             // traslada  
            a++;  
        } 
    }else if(R3 == -1){ //imprime un double
         printf((char*)p,D(R2));
    }else if(R3 == -2){//imprime un int
        printf((char*)p,I(R2));
    }else{ //sino, se imprime un salto de linea (siempre que en R2 este la dir a salto de linea)
        printf((char*)p,R2);
    }
...
// Enrtada: R1= Dir donde guardar el valor
            //R0= etiqueta de retorno
L scanf_: { 
    float a;//siempre lee unfloat
    scanf("%f",&a);             // traslada  
    RR1 = a;   
    D(R1) = RR1;                           
    GT(R0);                           // retorna
